...
\begin{figure}[h!]
	...
	\caption{An illustrator of genetic algorithm selection, mutation, and crossover for alloy design. As the algorithm proceeds, the selection step typically focuses in on a local minimum of `best' alloys for the intended outcome/application. The model assessment, or selection, step can vary depending on the alloy property being optimized. The model used should be able to predict material response based on composition.}
	\label{fig:GA}
\end{figure}
%
